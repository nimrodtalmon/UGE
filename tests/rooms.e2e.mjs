...
// the room's table runs the usual wizard; its session/QR carry the room path
...
await table.evaluate((code) => fetch(`/r/${code}/api/lobby/setup`, {
  method: 'POST', headers: { 'content-type': 'application/json' },
  body: JSON.stringify({ players: 1, phones: 1 }),
}), codeA);
await table.waitForSelector('h2:has-text("Pick a game")', { timeout: 10000 });
if (await table.locator('footer button:has-text("Update")').count()) fail('Update button visible in public mode');
if (!(await table.textContent('.room-code')).includes(codeA)) fail('table does not show the room code');
...
await r2.waitForSelector('.st-players', { timeout: 10000 });
await r2.evaluate((code) => fetch(`/r/${code}/api/lobby/setup`, {
  method: 'POST', headers: { 'content-type': 'application/json' },
  body: JSON.stringify({ players: 1, phones: 1 }),
}), codeB);
...
// a full game runs inside the room
